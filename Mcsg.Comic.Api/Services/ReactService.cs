    ...
    private async Task<ReactionInfor> GetReactionInfor(Guid targetId)
    {
        switch (typeof(T).Name)
        {
            case nameof(ComicPostReaction):
                var post = await GetData<ComicPost>(p => p.Id == targetId);
                if (post == null)
                {
            ...
            case nameof(ComicSubPostReaction):
                var subPost = await GetData<ComicSubPost>(p => p.Id == targetId);
                if (subPost == null)
                {
            ...
            case nameof(ComicPostCommentReaction):
                var commentPost = await GetData<ComicPostComment>(p => p.Id == targetId, q => q.Include(p => p.Post));
            ...
            case nameof(ComicSubPostCommentReaction):
                var commentSubPost = await GetData<ComicSubPostComment>(p => p.Id == targetId, q => q.Include(p => p.Post));
