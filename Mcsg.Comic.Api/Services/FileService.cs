...
public class FileService : IFileService
{
    ...
    public async Task<UploadFileDto> UploadFileAsync(FileCreateR request)
    {
        ...
        if (request.IsPublic == true)
        {
            ...
            objectName = $"{MinioFolder.Comic}/{user.UserFolder}{type}/{hashFileName}";
    ...
    /// <param name="dto">UploadResourceDto</param>
    /// <returns>Return the result</returns>
    public async Task<List<UploadFileDto>> ProcessComicFilesAsync(UploadResourceDto dto)
    {
    ...
    public async Task RemoveFileAsync(Guid postId, string userFolder)
    {
        ...
        if (resourcesDb.Any())
        {
            ...
            foreach (var resource in resourcesDb)
            {
                ...
                targetBlobName = $"{MinioFolder.Comic}/{targetBlobName}";
    ...
    private async Task<Tuple<List<ComicResource>, List<SubUploadFileDto>>> CompleteFilesAsync(UploadResourceDto dto, bool addSubPost)
    {
        ...
        foreach (var resource in resourceList)
        {
            ...
            var tempObjectName = $"{MinioFolder.Comic}/{tempBlobName}";
