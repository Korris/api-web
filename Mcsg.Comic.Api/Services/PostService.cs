...
public partial class PostService : IPostService
{
    ...
    {
        var queryNameListPost = $@"SELECT ""Title"" FROM ""comic"".""ComicPosts""  where ""Type"" != {(int)PostType.Feed}  AND ""IsDelete"" = false ";
    ...
    public async Task<ComicSubPost> SubPostChapterToSeries(string comicHashId, StoryChapterPostR chapterPostReq)
    {
        ...
        var newChapter = new ComicSubPost
        {
    ...
    public async Task<ComicSubPost> SubPostUpdateChapterToSeries(string postHashId, int order, StoryChapterPostR chapterPostReq)
    {
        var currentUserId = _currentUserService?.Session?.UserId;
        if (!chapterPostReq.IsPublicNow && chapterPostReq.PublishDate == null)
        {
        ...
        var newChapter = await _subPostRepository
            .Connection.QueryFirstAsync<ComicSubPost>(querySubpost, new
            {
    ...
    #region POST - COMMON
    private void VerifyPost(ComicPost post, bool checkCompleted)
    {
        var currentUserId = _currentUserService.Session.UserId;
        if (post != null)
        {
