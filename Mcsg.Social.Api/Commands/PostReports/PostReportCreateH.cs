...
public class PostReportCreateH : BaseH, IRequestHandler<PostReportCreateR, SingleResponse>
{
    ...
    public async Task<SingleResponse> Handle(PostReportCreateR request, CancellationToken cancellationToken)
    {
        ...
        var hasPostReport = await _context.SocialPostReports.AnyAsync(p => p.PostId == request.PostId && p.UserId == userId, cancellationToken);
        if (hasPostReport)
        {
            res.SetError(E115);
            return res;
        ...
        var ett = SocialPostReport.Create(request.PostId, userId, reasonType, request.ReasonText);
        await _context.SocialPostReports.AddAsync(ett, cancellationToken);
