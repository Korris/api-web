...
public partial class ReactService<T> : BaseSettingS, IReactService<T> where T : BaseReaction, new()
{
    ...
    public async Task<ReactionUpdateResponse> AddReaction(ReactionReactR request)
    {
        ...
        {
            case nameof(StoryPostReaction):
                postId = targetId;
                authorId = await _context.Available<StoryPost>(false)
                    .Where(p => p.Id == targetId)
                    .Select(p => p.UserId)
                    .FirstOrDefaultAsync();
                break;

            case nameof(StorySubPostReaction):
            ...
            case nameof(StoryPostCommentReaction):
                var commentPost = await _context.Available<StoryPostComment>(false)
                    .Where(p => p.Id == targetId)
                    .FirstOrDefaultAsync();
            ...
            case nameof(StorySubPostCommentReaction):
                var commentSubPost = await _context.Available<StorySubPostComment>(false)
                    .Include(p => p.Post)
                    .Where(p => p.Id == targetId)
                    .FirstOrDefaultAsync();
        ...
        {
            MicroService = MicroService.Story.ToString(),
            TargetId = postId,
            SubPostId = subPostId
        };
    ...
    public async Task<ReactionUpdateResponse> RemoveReaction(ReactionReactR request)
    {
        ...
        var postId = await _context.Available<StorySubPost>()
                                   .Where(p => p.Id == targetId)
                                   .Select(p => p.PostId)
                                   .FirstOrDefaultAsync();
    ...
    {
        return await _context.Available<StoryPost>(false)
            .Where(p => p.Id == postId)
            .Select(p => p.UserId)
            .FirstOrDefaultAsync();
    }
