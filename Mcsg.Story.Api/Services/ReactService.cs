    ...
    private async Task<ReactionInfor> GetReactionInfor(Guid targetId)
    {
        switch (typeof(T).Name)
        {
            case nameof(StoryPostReaction):
                var post = await GetData<StoryPost>(p => p.Id == targetId);
                if (post == null)
                {
            ...
            case nameof(StorySubPostReaction):
                var subPost = await GetData<StorySubPost>(p => p.Id == targetId);
                if (subPost == null)
                {
            ...
            case nameof(StoryPostCommentReaction):
                var commentPost = await GetData<StoryPostComment>(p => p.Id == targetId, q => q.Include(p => p.Post));
            ...
            case nameof(StorySubPostCommentReaction):
                var commentSubPost = await GetData<StorySubPostComment>(p => p.Id == targetId, q => q.Include(p => p.Post));
