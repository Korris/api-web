...
public class FileService : IFileService
{
    ...
    public async Task<UploadFileDto> UploadFileAsync(FileCreateR request)
    {
        ...
        if (request.IsPublic == true)
        {
            ...
            objectName = $"{MinioFolder.Story}/{user.UserFolder}{type}/{hashFileName}";
    ...
    public async Task<List<SubUploadFileDto>> UpdateFilesAsync(UploadResourceDto dto)
    {
        ...
        var subPostDB = await _context.Available<StorySubPost>().Where(p => p.PostId == dto.PostId).ToListAsync();
        ...
        var subpostAndResourceHashId = await (from a in _context.Available<StorySubPost>()
                                              join b in _context.Available<StoryResource>()
    ...
    public async Task RemoveFileAsync(Guid postId, string userFolder)
    {
        ...
        if (resourcesDb.Any())
        {
            ...
            foreach (var resource in resourcesDb)
            {
                ...
                tempBlobName = $"{MinioFolder.Story}/{tempBlobName}";
                ...
                targetBlobName = $"{MinioFolder.Story}/{targetBlobName}";
    ...
    private async Task<Tuple<List<StoryResource>, List<SubUploadFileDto>>> CompleteFilesAsync(UploadResourceDto dto, bool addSubPost)
    {
        ...
        var resourceDb = await _context.Available<StoryResource>().Where(p => p.SubPostId == dto.SubPostId).ToListAsync();
        var resourceHashIdRemove = resourceDb.Where(p => !hashIds.Contains(p.HashId)).Select(p => p.HashId).ToList();
        var resourceList = await _context.Available<StoryResource>().Where(p => hashIds.Contains(p.HashId)).ToListAsync();
        ...
        foreach (var resource in resourceList)
        {
            ...
            var tempObjectName = $"{MinioFolder.Story}/{tempBlobName}";
            var isExistTempFile = await _sc.GetStrategy(resource.MinioInstance).StatObject(tempObjectName, null);

            var targetObjectName = $"{MinioFolder.Story}/{targetBlobName}";
    ...
    {
        return from a in _context.Available<StoryResource>()
    ...
    private async Task RemoveResource(List<string?> hashIds, List<Guid?>? subPostIds)
    {
        ...
        if (hashIds?.Count > 0)
        {
            var a = await _context.Available<StoryResource>().Where(p => hashIds.Contains(p.HashId)).ToListAsync();
        ...
        {
            var b = await _context.Available<StorySubPost>().Where(p => subPostIds.Contains(p.Id)).ToListAsync();
