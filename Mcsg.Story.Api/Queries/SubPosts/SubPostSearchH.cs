...
public class SubPostSearchH : BaseSettingH, IRequestHandler<SubPostSearchR, SingleResponse>
{
    ...
    public async Task<SingleResponse> Handle(SubPostSearchR request, CancellationToken cancellationToken)
    {
        ...
        var q = _context.Available<StorySubPost>(false).Where(p => p.PostId == postId)
            .Select(subPost => new ChapterResponse
            {
                ...
                CommentCount = _context.Available<StorySubPostComment>(false).Count(p => p.PostId == subPost.Id)
            });
    ...
    /// <returns>Return the result</returns>
    private async Task<StorySubSortedSearchRsp> SortedSearch(Guid postId, bool descending, int pagNum, int pageSize, List<Guid> ids)
    {
        var res = new StorySubSortedSearchRsp();
        ...
        try
        {
            ...
            var req = new StorySubSortedSearchReq
            {
