...
public class PostReportCreateH : BaseH, IRequestHandler<PostReportCreateR, SingleResponse>
{
    ...
    public async Task<SingleResponse> Handle(PostReportCreateR request, CancellationToken cancellationToken)
    {
        ...
        var hasPostReport = await _context.StoryPostReports.AnyAsync(p => p.PostId == request.PostId && p.UserId == userId, cancellationToken);
        if (hasPostReport)
        {
        ...
        var ett = StoryPostReport.Create(request.PostId, userId, reasonType, request.ReasonText);
        await _context.StoryPostReports.AddAsync(ett, cancellationToken);
