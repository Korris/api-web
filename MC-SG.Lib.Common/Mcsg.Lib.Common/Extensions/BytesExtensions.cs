namespace Mcsg.Lib.Common.Extensions
{
    public static class BytesExtensions
    {
        private enum Kinds
        {
            Bytes = 0,
            Kilobytes = 1,
            Megabytes = 2,
            Gigabytes = 3,
            Terabytes = 4,
        }

        public static double FromKilobytes(this double kilobytes) =>
            From(kilobytes, Kinds.Kilobytes);

        public static double FromMegabytes(this double megabytes) =>
            From(megabytes, Kinds.Megabytes);

        public static double FromGigabytes(this double gigabytes) =>
            From(gigabytes, Kinds.Gigabytes);

        public static double FromTerabytes(this double terabytes) =>
            From(terabytes, Kinds.Terabytes);

        public static double ToKilobytes(this double bytes) =>
            To(bytes, Kinds.Kilobytes);

        public static double ToMegabytes(this double bytes) =>
            To(bytes, Kinds.Megabytes);

        public static double ToGigabytes(this double bytes) =>
            To(bytes, Kinds.Gigabytes);

        public static double ToTerabytes(this double bytes) =>
            To(bytes, Kinds.Terabytes);

        private static double To(double value, Kinds kind) =>
            value / Math.Pow(1024, (int)kind);

        private static double From(double value, Kinds kind) =>
            value * Math.Pow(1024, (int)kind);
    }

}
