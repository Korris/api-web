...
public partial class DocumentCommentService : BaseS, IDocumentCommentService
{
    /// <summary>
    /// Initialize
    /// </summary>
    ...
    public DocumentCommentService(IMcsgContext context, IBusinessText businessText, IRepository<DocumentPostComment> postCommentRepository, IRepository<DocumentSubPostComment> subPostCommentRepository, IRepository<DocumentResource> resourceRepository, IRepository<Mention> mentionRepository, IResourceCommentService resourceCommentService, INotificationService notificationService, IMentionService mentionService, ISmartCountService smartCountService, IMapper mapper) : base(context)
    {
    ...
    public async Task<PostCommentResp> PostComment(PostCommentReq req)
    {
        ...
        {
            var post = await _context.Available<DocumentPost>().FirstOrDefaultAsync(p => p.Id == req.PostId);
            if (post != null)
            {
                pDto.Id = post.Id;
        ...
        {
            var subPost = await _context.Available<DocumentSubPost>().FirstOrDefaultAsync(p => p.Id == req.PostId);
            if (subPost != null)
            {
                order = subPost.Order;
                pDto.Id = subPost.Id;
        ...
        if (!string.IsNullOrEmpty(pDto.HashId) && !receiverIds.Contains(pDto.CreateBy))
        {
            ...
            if (commentNotiRequest.Type == "subpost")
            {
                ...
                var post = await _context.Available<DocumentPost>().FirstOrDefaultAsync(p => p.Id == response.PostIdOfPost);
        ...
        if (receiverIds.Count > 0)
        {
            await _notificationService.AddPostMentionNotification(new MentionPostNotificationReq
            {
                ...
                UserId = userId.Value,
                EntityType = req.Type == PostTypes.Post ? NotificationEntityType.DocumentPostCommentMention : NotificationEntityType.DocumentSubPostCommentMention
            });
        }
    ...
    public async Task<PostCommentResp> UpdateComment(UpdateCommentReq req)
    {
        ...
        var ett = req.Type == "post"
                ? await _context.Available<DocumentPostComment>()
                        .Where(p => p.Id == req.CommentId)
                        .Select(p => new { p.CreatedBy })
                        .FirstOrDefaultAsync()
                : await _context.Available<DocumentSubPostComment>()
                        .Where(p => p.Id == req.CommentId)
                        .Select(p => new { p.CreatedBy })
                        .FirstOrDefaultAsync();
    ...
    {
        var comment = new DocumentPostComment
        {
            AuthorId = author.Id,
        ...
        await _context.DocumentPostComments.AddAsync(comment);
    ...
    {
        var comment = new DocumentSubPostComment
        {
            AuthorId = author.Id,
            Body = req.CommentText.RemoveMaliciousText(),
            CreatedBy = author.Id,
        ...
        await _context.DocumentSubPostComments.AddAsync(comment);
        await _context.SaveChangesAsync(default);
    ...
    {
        var comment = await _context.Available<DocumentPostComment>()
    ...
    {
        var comment = await _context.Available<DocumentSubPostComment>()
            .Include(p => p.Resource)
            .FirstOrDefaultAsync(p => p.Id == req.CommentId);
        if (comment == null)
        {
    ...
    {
        var comment = await _context.Available<DocumentPostComment>().FirstOrDefaultAsync(p => p.Id == req.CommentId);
        if (comment == null)
        {
            throw new NotFoundException(RealtimeErrorCode.NotFoundComment, RealtimeErrorMessage.NotFoundComment);
        }
    ...
    {
        var comment = await _context.Available<DocumentSubPostComment>().FirstOrDefaultAsync(p => p.Id == req.CommentId);
        if (comment == null)
        {
            throw new NotFoundException(RealtimeErrorCode.NotFoundComment, RealtimeErrorMessage.NotFoundComment);
        }
        ...
        var post = await _context.Available<DocumentSubPost>().FirstOrDefaultAsync(p => p.Id == comment.PostId);
