...
public partial class DocumentCommentService : BaseS, IDocumentCommentService
{
    ...
    public async Task<PostCommentResp> PostComment(PostCommentReq req)
    {
        ...
        {
            var post = await _context.DocumentPostAvailable.FirstOrDefaultAsync(p => p.Id == req.PostId);
            if (post != null)
            {
                pDto.Id = post.Id;
        ...
        {
            var subPost = await _context.DocumentSubPostAvailable.FirstOrDefaultAsync(p => p.Id == req.PostId);
            if (subPost != null)
            {
                order = subPost.Order;
                pDto.Id = subPost.Id;
        ...
        if (!string.IsNullOrEmpty(pDto.HashId) && !receiverIds.Contains(pDto.CreateBy))
        {
            ...
            if (commentNotiRequest.Type == "subpost")
            {
                ...
                var post = await _context.DocumentPostAvailable.FirstOrDefaultAsync(p => p.Id == response.PostIdOfPost);
    ...
    public async Task<PostCommentResp> UpdateComment(UpdateCommentReq req)
    {
        ...
        var ett = req.Type == "post"
                ? await _context.DocumentPostCommentAvailable
                        .Where(p => p.Id == req.CommentId)
                        .Select(p => new { p.CreatedBy })
                        .FirstOrDefaultAsync()
                : await _context.DocumentSubPostCommentAvailable
                        .Where(p => p.Id == req.CommentId)
                        .Select(p => new { p.CreatedBy })
                        .FirstOrDefaultAsync();
    ...
    {
        var comment = await _context.DocumentPostCommentAvailable.FirstOrDefaultAsync(p => p.Id == req.CommentId);
        if (comment == null)
        {
    ...
    {
        var comment = await _context.DocumentSubPostCommentAvailable.FirstOrDefaultAsync(p => p.Id == req.CommentId);
        if (comment == null)
        {
            throw new NotFoundException(RealtimeErrorCode.NotFoundComment, RealtimeErrorMessage.NotFoundComment);
        }
    ...
    private async Task<PostCommentResp> DeleteCommentInSubPost(DeleteCommentReq req)
    {
        ...
        var post = await _context.DocumentSubPostAvailable.FirstOrDefaultAsync(p => p.Id == comment.PostId);
