...
public class ReportCreateH : BaseH, IRequestHandler<ReportCreateR, SingleResponse>
{
    ...
    public async Task<SingleResponse> Handle(ReportCreateR request, CancellationToken cancellationToken)
    {
        ...
        {
            // Create Report
            ett = StoryReport.Create(entityId, type, userId);
            await _context.StoryReports.AddAsync(ett, cancellationToken);
        }

        // Create ReportDetail
        var reasonType = request.ReasonType.ToEnum(ReasonType.Other);
        var ettDetail = StoryReportDetail.Create(ett.Id, reasonType, request.ReasonText, userId);
        await _context.StoryReportDetails.AddAsync(ettDetail, cancellationToken);

        await _context.SaveChangesAsync(default);

        // Update Status
        await _context.StoryReports.Where(p => p.Id == ett.Id).ExecuteUpdateAsync(p => p.SetProperty(q => q.Status, ReportStatus.Reviewing), cancellationToken);
