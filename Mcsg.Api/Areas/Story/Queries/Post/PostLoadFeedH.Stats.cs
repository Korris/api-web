...
public partial class PostLoadFeedH
{
    ...
    {
        var qReaction = _context.Available<StoryPostReaction>(false).Where(r => postIds.Contains(r.TargetId));
    ...
    {
        var qPostComment = _context.Available<StoryPostComment>(false);
        ...
        var qSubPostComment = _context.Available<StorySubPostComment>(false);
        var subPostComments = await (
            from a in qSubPostComment
            join b in _context.Available<StorySubPost>(false) on a.PostId equals b.Id
