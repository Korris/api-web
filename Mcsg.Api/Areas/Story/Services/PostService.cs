...
using Mcsg.Api.Areas.Story.Constants;
...
public partial class PostService : BaseMinioS, IPostService
{
    ...
    public async Task<PostSeriesQueryDbResponse> GetSeries(StoryHashIdR req)
    {
    ...
    public async Task<PagedResponse<PostSeriesTopResponse>> GetTopSeriesAsync(PostType type, StoryPostListSeriesR request)
    {
        ...
        {
            q = q.Where(p => p.StoryTagPosts.Select(p => p.Tag.Name).Contains(request.HashTag));
        }

        var dbFeed = await q.Include(p => p.User)
            .Include(p => p.StorySubPosts)
            .Include(p => p.StoryTagPosts).ThenInclude(p => p.Tag)
            .Select(p => new PostSeriesTopQueryDbResponse
            {
                ...
                LatestCreatedOn = p.StorySubPosts.Max(p => p.PublishDate),
                Tags = p.StoryTagPosts.Select(p => p.Tag.Name).ToArray(),
                Status = p.Status,
                Hide = p.Hide
            })
    ...
    /// </summary>
    /// <param name="post">The story post containing the ThumbnailUrl and CoverUrl.</param>
    ...
    private async Task<CommentsResult> GetMostCommentReaction(List<string> hashIds)
    {
        ...
        var fn = "story.fw_get_most_reaction_comments";
        ...
        fn = "story.fw_get_total_comment_counts";
