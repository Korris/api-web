...
/// <summary>
/// Reactions for game posts / comments. Trimmed copy of Comic ReactService: same upsert rule
/// (one row per user per target, re-react changes the type), no notifications, no smart counts.
/// </summary>
public class GameReactService : IGameReactService
{
    #region -- Methods --

    public GameReactService(IMcsgContext context)
    {
        _context = context;
    }
    ...
    {
        await EnsurePostVisibleAsync(request.TargetId, request.UserId);
        return await UpsertAsync(_context.GamePostReactions, request);
    }

    public async Task<ReactionSummaryResponse> RemovePostReactionAsync(ReactionReactR request)
    {
        return await RemoveAsync(_context.GamePostReactions, request);
    }

    public async Task<ReactionSummaryResponse> ReactToCommentAsync(ReactionReactR request)
    {
        var exists = await _context.Available<GamePostComment>(false)
        ...
        return await UpsertAsync(_context.GamePostCommentReactions, request);
    }

    public async Task<ReactionSummaryResponse> RemoveCommentReactionAsync(ReactionReactR request)
    {
        return await RemoveAsync(_context.GamePostCommentReactions, request);
    }

    public Task<Dictionary<Guid, ReactionSummaryResponse>> GetPostSummariesAsync(IEnumerable<Guid> postIds, Guid? userId)
    {
        return SummariesAsync(_context.GamePostReactions, postIds, userId);
    }

    public Task<Dictionary<Guid, ReactionSummaryResponse>> GetCommentSummariesAsync(IEnumerable<Guid> commentIds, Guid? userId)
    {
        return SummariesAsync(_context.GamePostCommentReactions, commentIds, userId);
    }
    ...
    /// <summary>
    /// Post must exist and be public, unless the reacting user owns it
    /// </summary>
    private async Task EnsurePostVisibleAsync(Guid postId, Guid? userId)
    {
        var ok = await _context.Available<GamePost>(false)
            .AnyAsync(p => p.Id == postId && (p.Status == PostStatus.Public || p.UserId == userId));
        if (!ok)
        {
            throw new NotFoundException(nameof(E204), E204);
        }
    ...
    private async Task<ReactionSummaryResponse> SummaryAsync<T>(DbSet<T> set, Guid targetId, Guid? userId) where T : BaseReaction
    {
