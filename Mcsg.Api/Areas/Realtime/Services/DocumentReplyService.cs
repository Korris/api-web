...
public partial class DocumentReplyService : BaseS, IDocumentReplyService
{
    ...
    public DocumentReplyService(IMcsgContext context, IBusinessText businessText, IRepository<DocumentPostComment> postCommentRepository, IRepository<DocumentSubPostComment> subPostCommentRepository, IResourceCommentService resourceCommentService, IRepository<DocumentResource> resourceRepository, IRepository<Mention> mentionRepository, INotificationService notificationService, IMentionService mentionService, IMapper mapper) : base(context)
    {
    ...
    {
        var comment = await _context.Available<DocumentPostComment>().FirstOrDefaultAsync(p => p.Id == req.ReplyCommentId);
        if (comment == null)
        {
    ...
    {
        var comment = await _context.Available<DocumentSubPostComment>().FirstOrDefaultAsync(p => p.Id == req.ReplyCommentId);
        if (comment == null)
        {
            throw new NotFoundException(RealtimeErrorCode.NotFoundComment, RealtimeErrorMessage.NotFoundComment);
        }
    ...
    private async Task<ReplyCommentResp> DeleteReplyToSubPostComment(DeleteReplyCommentReq req)
    {
        ...
        var postIdOfPost = await _context.DocumentSubPosts
