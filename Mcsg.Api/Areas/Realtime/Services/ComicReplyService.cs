...
public partial class ComicReplyService : BaseS, IComicReplyService
{
    /// <summary>
    /// Initialize
    /// </summary>
    public ComicReplyService(
    ...
    public async Task<ReplyCommentResp> ReplyComment(ReplyCommentReq req)
    {
        ...
        if (userId == null) throw new NotFoundException(nameof(E303), E303);
        if (!ValidReplyComment(req)) throw new NotFoundException(RealtimeErrorCode.InvalidRequest, RealtimeErrorCode.InvalidRequest);
        ...
        if (req.Type == PostTypes.Post)
        {
            ...
            if (post != null)
            {
                pDto.Id = post.Id; pDto.HashId = post.HashId;
        ...
        else
        {
            ...
            {
                order = subPost.Order;
                pDto.Id = subPost.Id; pDto.HashId = subPost.HashId;
    ...
    public async Task<ReplyCommentResp> UpdateReplyComment(UpdateReplyCommentReq req)
    {
        ...
        var ett = req.Type == "post"
            ? await _context.Available<ComicPostComment>().Where(p => p.Id == req.ReplyCommentId).Select(p => new { p.CreatedBy }).FirstOrDefaultAsync()
            : await _context.Available<ComicSubPostComment>().Where(p => p.Id == req.ReplyCommentId).Select(p => new { p.CreatedBy }).FirstOrDefaultAsync();

        if (ett == null) throw new NotFoundException(nameof(E204), E204);
        if (ett.CreatedBy != userId) throw new ForbiddenAccessException(nameof(E309), E309);
    ...
    public async Task<ReplyCommentResp> DeleteReplyComment(DeleteReplyCommentReq req)
    {
        var userId = req.UserId;
        if (userId == null) throw new NotFoundException(nameof(E303), E303);
        if (req.ReplyCommentId == Guid.Empty) throw new NotFoundException(RealtimeErrorCode.InvalidRequest, RealtimeErrorCode.InvalidRequest);

        return req.Type == PostTypes.Post
            ? await DeleteReplyToPostComment(req)
            : await DeleteReplyToSubPostComment(req);
    }

    #region Add New Reply Comment
    ...
    private async Task<ReplyCommentResp> ReplyToPostComment(ReplyCommentReq req, AuthorDto author, ResourceCommentResp resource, PostDto post)
    {
        ...
        {
            AuthorId = author.Id, Body = req.ReplyText.RemoveMaliciousText(), CreatedBy = author.Id,
            ParentId = req.ReplyToCommentId, ModifiedBy = author.Id, PostId = req.PostId,
            Status = CommentStatus.Public, ResourceId = resource?.Id ?? null, GifId = req.GifId,
            QuoteId = req?.QuoteId == Guid.Empty ? null : req.QuoteId, CustomNote = req.CustomNote.RemoveMaliciousText()
        };
        ...
        {
            PostId = comment.PostId, ReplyText = comment.Body, ReplyDate = comment.CreatedOn, Type = PostTypes.Post,
            Id = comment.Id, ReplyToCommentId = comment.ParentId.Value, ResourceHashId = resource?.HashId ?? null,
            ResourceUrl = resource?.Url ?? null, AuthorId = comment.AuthorId, GifId = comment.GifId, Mentions = req.Mentions,
    ...
    private async Task<ReplyCommentResp> ReplyToSubPostComment(ReplyCommentReq req, AuthorDto author, ResourceCommentResp resource, PostDto post)
    {
        var comment = new ComicSubPostComment
        {
            ...
            ParentId = req.ReplyToCommentId, ModifiedBy = author.Id, PostId = req.PostId,
            Status = CommentStatus.Public, ResourceId = resource?.Id ?? null, GifId = req.GifId,
            CustomNote = req.CustomNote.RemoveMaliciousText(), QuoteId = req?.QuoteId == Guid.Empty ? null : req.QuoteId
        };
        ...
        return new ReplyCommentResp
        {
            PostId = comment.PostId, ReplyText = comment.Body, ReplyDate = comment.CreatedOn, Type = PostTypes.SubPost,
            Id = comment.Id, ReplyToCommentId = comment.ParentId.Value, ResourceHashId = resource?.HashId ?? null,
            ResourceUrl = resource?.Url ?? null, AuthorId = comment.AuthorId, GifId = comment.GifId, Mentions = req.Mentions,
            QuoteId = comment?.QuoteId == Guid.Empty ? null : comment.QuoteId
        };
    ...
    {
        var comment = await _context.Available<ComicPostComment>().FirstOrDefaultAsync(p => p.Id == req.ReplyCommentId);
        if (comment == null) throw new NotFoundException(RealtimeErrorCode.NotFoundComment, RealtimeErrorMessage.NotFoundComment);
        if (comment.AuthorId != author.Id) throw new NotFoundException(RealtimeErrorCode.UnAuthorizeUpdate, RealtimeErrorMessage.UnAuthorizeUpdate);
    ...
    {
        var comment = await _context.Available<ComicSubPostComment>().FirstOrDefaultAsync(p => p.Id == req.ReplyCommentId);
        if (comment == null) throw new NotFoundException(RealtimeErrorCode.NotFoundComment, RealtimeErrorMessage.NotFoundComment);
        if (comment.AuthorId != author.Id) throw new NotFoundException(RealtimeErrorCode.UnAuthorizeUpdate, RealtimeErrorMessage.UnAuthorizeUpdate);
    ...
    private async Task<ReplyCommentResp> DeleteReplyToPostComment(DeleteReplyCommentReq req)
    {
        var comment = await _context.Available<ComicPostComment>().FirstOrDefaultAsync(p => p.Id == req.ReplyCommentId);
        if (comment == null) throw new NotFoundException(RealtimeErrorCode.NotFoundComment, RealtimeErrorMessage.NotFoundComment);
        if (comment.AuthorId != req.UserId) throw new NotFoundException(RealtimeErrorCode.UnAuthorizeUpdate, RealtimeErrorMessage.UnAuthorizeUpdate);
        ...
        await _postCommentRepository.Connection.ExecuteAsync(command, new
        {
            Id = req.ReplyCommentId, ModifiedBy = req.UserId, ModifiedOn = DateTime.UtcNow,
            LocationType = (int)MentionLocationType.PostCommentReply
        });

        return new ReplyCommentResp { PostId = comment.PostId, ReplyDate = comment.CreatedOn, Type = PostTypes.Post, Id = comment.Id, ReplyToCommentId = comment.ParentId.Value };
    }
    ...
    private async Task<ReplyCommentResp> DeleteReplyToSubPostComment(DeleteReplyCommentReq req)
    {
        ...
        var postIdOfPost = await _context.ComicSubPosts.Where(p => p.Id == comment.PostId).Select(p => p.PostId).FirstOrDefaultAsync();
        ...
        await _subPostCommentRepository.Connection.ExecuteAsync(command, new
        {
            Id = req.ReplyCommentId, ModifiedBy = req.UserId, ModifiedOn = DateTime.UtcNow,
            LocationType = (int)MentionLocationType.SubPostCommentReply
        });

        return new ReplyCommentResp { PostId = comment.PostId, ReplyDate = comment.CreatedOn, Type = PostTypes.SubPost, Id = comment.Id, ReplyToCommentId = comment.ParentId.Value, PostIdOfPost = postIdOfPost };
    }
    ...
    {
        if (req.PostId == Guid.Empty) throw new NotFoundException(RealtimeErrorCode.InvalidPostId, RealtimeErrorMessage.InvalidPostId);
        if (req.ReplyToCommentId == Guid.Empty) throw new NotFoundException(RealtimeErrorCode.InvalidReplyToCommentId, RealtimeErrorMessage.InvalidReplyToCommentId);
        if (string.IsNullOrWhiteSpace(req.ReplyText + req.ResourceHashId + req.GifId)) throw new NotFoundException(RealtimeErrorCode.InvalidMessage, RealtimeErrorMessage.InvalidMessage);
        return true;
    }
