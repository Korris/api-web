...
public partial class ComicCommentService : BaseS, IComicCommentService
{
    ...
    public async Task<PostCommentResp> PostComment(PostCommentReq req)
    {
        ...
        if (!ValidComment(req)) throw new NotFoundException(RealtimeErrorCode.InvalidRequest, RealtimeErrorCode.InvalidRequest);
    ...
    public async Task<PostCommentResp> UpdateComment(UpdateCommentReq req)
    {
        ...
        var ett = req.Type == "post"
            ? await _context.Available<ComicPostComment>().Where(p => p.Id == req.CommentId).Select(p => new { p.CreatedBy }).FirstOrDefaultAsync()
            : await _context.Available<ComicSubPostComment>().Where(p => p.Id == req.CommentId).Select(p => new { p.CreatedBy }).FirstOrDefaultAsync();
    ...
    public async Task<PostCommentResp> DeleteComment(DeleteCommentReq req)
    {
        var userId = req.UserId;
        if (userId == null) throw new NotFoundException(nameof(E303), E303);
        if (req.CommentId == Guid.Empty) throw new NotFoundException(RealtimeErrorCode.InvalidRequest, RealtimeErrorCode.InvalidRequest);

        return req.Type == PostTypes.Post
            ? await DeleteCommentInPost(req)
            : await DeleteCommentInSubPost(req);
    }
    ...
    private async Task<PostCommentResp> CommentToPost(PostCommentReq req, AuthorDto author, ResourceCommentResp resource, PostDto post)
    {
        ...
        {
            PostId = comment.PostId, CommentText = comment.Body, CommentDate = comment.CreatedOn,
            Type = PostTypes.Post, Id = comment.Id, ResourceHashId = resource?.HashId ?? null,
            ResourceUrl = resource?.Url ?? null, AuthorId = comment.AuthorId, GifId = req.GifId, Mentions = req.Mentions
        };
    }
    ...
    private async Task<PostCommentResp> CommentToSubPost(PostCommentReq req, AuthorDto author, ResourceCommentResp resource, PostDto post)
    {
        ...
        {
            PostId = comment.PostId, CommentText = comment.Body, CommentDate = comment.CreatedOn,
            Type = PostTypes.SubPost, Id = comment.Id, ResourceHashId = resource?.HashId ?? null,
            ResourceUrl = resource?.Url ?? null, AuthorId = comment.AuthorId, GifId = req.GifId, Mentions = req.Mentions
        };
    }
    ...
    {
        var comment = await _context.Available<ComicPostComment>().Include(p => p.Resource).FirstOrDefaultAsync(p => p.Id == req.CommentId);
        ...
        if (comment.Resource != null) comment.Resource.IsDelete = true;
    ...
    {
        var comment = await _context.Available<ComicSubPostComment>().Include(p => p.Resource).FirstOrDefaultAsync(p => p.Id == req.CommentId);
        if (comment == null) throw new NotFoundException(RealtimeErrorCode.NotFoundComment, RealtimeErrorMessage.NotFoundComment);
        if (comment.AuthorId != author.Id) throw new NotFoundException(RealtimeErrorCode.UnAuthorizeUpdate, RealtimeErrorMessage.UnAuthorizeUpdate);
    ...
    private async Task<PostCommentResp> DeleteCommentInPost(DeleteCommentReq req)
    {
        ...
        {
            Id = req.CommentId, ModifiedBy = req.UserId, ModifiedOn = DateTime.UtcNow,
        ...
        return new PostCommentResp { PostId = comment.PostId, CommentDate = comment.CreatedOn, Type = PostTypes.Post, Id = comment.Id };
    }
    ...
    private async Task<PostCommentResp> DeleteCommentInSubPost(DeleteCommentReq req)
    {
        ...
        return new PostCommentResp { PostIdOfPost = post.PostId, PostId = comment.PostId, CommentDate = comment.CreatedOn, Type = PostTypes.SubPost, Id = comment.Id };
    }
    ...
    private bool ValidComment(PostCommentReq req)
    {
        ...
        if (string.IsNullOrWhiteSpace(req.CommentText + req.ResourceHashId + req.GifId)) throw new NotFoundException(RealtimeErrorCode.InvalidMessage, RealtimeErrorMessage.InvalidMessage);
        return true;
    }
