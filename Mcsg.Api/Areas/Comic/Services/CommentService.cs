...
public partial class CommentService : ICommentService
{
    ...
    public async Task<CommentResponse> GetCommentById(Guid commentId, bool isSubPost, Guid? userId, Guid? replyCommentId)
    {
        ...
        query = query.Replace("@CommentSource", $@"comic.""{(isSubPost ? "ComicSubPostComments" : "ComicPostComments")}""");
        ...
        if (comModel != null)
        {
            ...
            var tableName = isSubPost ? $@"comic.""ComicSubPostCommentReactions""" : $@"comic.""ComicPostCommentReactions""";
            var postCommentReactionResponse = await _postCommentRepository.Connection.QueryAsync<CommentReactionResponseQuery>(string.Format(ReactionExtension.GetReactionByTargetIdsQuery, tableName), new
            {
                TargetIds = targetIds,
                UserId = userId
    ...
    public async Task<bool> CheckPostExisted(CommentCheckPostExistedR request)
    {
        ...
        {
            return await _context.Available<ComicSubPost>().AnyAsync(p => p.Id == request.PostId);
        }
        else
        {
            return await _context.Available<ComicPost>().AnyAsync(p => p.Id == request.PostId);
        }
    }
