...
public partial class PostLinkService : IPostLinkService
{
    ...
    public async Task<PostLinkDto> AddLinkAsync(Guid postId, string content)
    {
        ...
        if (links.Any())
        {
            ...
            if (linkIndexs != null && linkIndexs.Any())
            {
                ...
                var postLink = new ComicPostLink
                {
                ...
                await _context.ComicPostLinks.AddAsync(postLink);
    ...
    {
        var postLinks = await _context.Available<ComicPostLink>().Where(p => p.PostId == postId).ToListAsync();
