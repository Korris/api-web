...
public partial class PostService : BaseMinioS, IPostService
{
    ...
    public async Task<FavoritePostResponse> FollowPost(IdBaseR request)
    {
        ...
        var ett = await _context.ComicPostFavorites
              .Where(p => p.CreatedBy == user.Id && p.PostId == postId)
              .FirstOrDefaultAsync();

        if (ett == null)
        {
            ett = new ComicPostFavorite
            {
                PostId = postId,
                UserId = user.Id,
                CreatedBy = user.Id
            };
            await _context.ComicPostFavorites.AddAsync(ett);
        }
        else
        {
            ett.IsDelete = !ett.IsDelete;
            _context.ComicPostFavorites.Update(ett);
        }
    ...
    public async Task<PagedResponse<ChapterTOCResponse>> GetChaptersListSimple(ComicHashIdR request)
    {
    ...
    /// </summary>
    /// <param name="post">The comic post containing the ThumbnailUrl and CoverUrl.</param>
    ...
    private async Task HandleThumbnailUrlAndCoverUrl(ComicPost post, bool isDelete, Guid userId)
    {
    ...
    private async Task<IEnumerable<MostReactionCommentResponse>> GetReactionAndMentionOfComment(IEnumerable<MostReactionCommentResponse>? comments, Guid? userId)
    {
        ...
        var queryPostCommentReaction = string.Format(ReactionExtension.GetReactionByTargetIdsQuery, $@"comic.""ComicPostCommentReactions""");
        ...
        var querySubPostCommentReaction = string.Format(ReactionExtension.GetReactionByTargetIdsQuery, $@"comic.""ComicSubPostCommentReactions""");
    ...
    #region -- SubPost --
    private async Task<ComicSubCreateRsp> SyncCreateSubToAna(ComicSubPost ett)
    {
    ...
    private async Task<ComicSubUpdateRsp> SyncUpdateSubToAna(ComicSubPost ett)
    {
        var res = new ComicSubUpdateRsp() { Success = true };
        ...
        try
        {
            ...
            var request = new ComicSubUpdateReq
            {
                SubPostId = ett.Id.ToString(),
                Permission = (int)ett.Permission,
                ModifiedOn = ett.ModifiedOn == null ? null : ett.ModifiedOn.ToString(),
    ...
    private async Task<ComicSubDeleteRsp> SyncDeleteSubToAna(Guid id)
    {
    ...
    private async Task<bool> SendAddSubPostNotificationAsync(ComicSubPost subPost, ComicPost post, bool isPremium)
    {
