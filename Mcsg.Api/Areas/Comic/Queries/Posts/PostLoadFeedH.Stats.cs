...
public partial class PostLoadFeedH
{
    ...
    {
        var qReaction = _context.Available<ComicPostReaction>(false).Where(r => postIds.Contains(r.TargetId));
    ...
    {
        var qPostComment = _context.Available<ComicPostComment>(false);
        ...
        var qSubPostComment = _context.Available<ComicSubPostComment>(false);
        var subPostComments = await (
            from a in qSubPostComment
            join b in _context.Available<ComicSubPost>(false) on a.PostId equals b.Id
