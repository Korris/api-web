...
public partial class PostLoadFeedH : BaseMinioH, IRequestHandler<PostLoadFeedR, SingleResponse>
{
    ...
    public async Task<SingleResponse> Handle(PostLoadFeedR request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                q = q.Where(p => p.ComicTagPosts.Any(tp => (tp.Tag.Name + "") == hashtag));
            }
            ...
            if (request.Hides != null && request.Hides.Count > 0)
            {
                ...
                q = q.Where(p => p.ComicSubPosts.Any(sp => !sp.IsDelete));
            }
            ...
            q = q.Where(p => !p.ComicPostHides.Any(h => h.UserId == request.UserId && !h.IsDelete));

            q = q.OrderByDescending(p => p.ComicSubPosts.Max(sp => sp.CreatedOn) > p.CreatedOn
                ? p.ComicSubPosts.Max(sp => sp.CreatedOn)
                : p.CreatedOn);

            // Favorite
            if (request.IsFavorite && request.UserId != null)
            {
                q = q.Where(p => p.ComicPostFavorites.Any(f => f.UserId == request.UserId && !f.IsDelete));
            }
            #endregion
