...
namespace Mcsg.Api.Areas.Comic.Queries;
...
using Mcsg.Api.Areas.Comic.Filters;
...
/// Handler
/// </summary>
public class MyComicSearchH : BaseSettingH, IRequestHandler<MyComicSearchR, SingleResponse>
{
    #region -- Methods --
    ...
    public MyComicSearchH(IMcsgContext context, ISetting setting) : base(context, setting) { }
    ...
    public async Task<SingleResponse> Handle(MyComicSearchR request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var q = _context.Available<ComicPost>().AsNoTracking().Where(p => p.UserId == userId);
            ...
            List<Guid> postIds = [];
            var viewResponse = new ComicPostViewRsp();
                ...
                foreach (var i in request.Sort)
                {
                    ...
                    else if (i.Field.Equals("NumberOfViews", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (viewResponse != null && viewResponse.Items != null)
                        {
                            viewCounts = viewResponse.Items.ToDictionary(v => v.PostId + "", v => v.NumberOfViews);
                            listIds = viewResponse.Items.Select(v => v.PostId).ToList();
                            q = _context.Available<ComicPost>().AsNoTracking().Where(p => p.UserId == userId);
    ...
    private async Task<ComicPostViewRsp> GetPostViewFrAna(Guid userId, int pageNumber, int pageSize, bool isAscending, string? keyWord, List<Guid> postIds, List<PostPermission> permissions, DateTime? frDate, DateTime? toDate)
    {
        var res = new ComicPostViewRsp();
        ...
        try
        {
            ...
            var request = new ComicPostViewReq
            {
