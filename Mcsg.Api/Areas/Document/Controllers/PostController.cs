...
public class PostController : ControllerBase
{
    ...
    public async Task<IActionResult> GetTopListHitDocument(PostType postType, string tagName, [FromQuery] DocumentTopPostR loadReq)
    {
        if (postType != PostType.Story && postType != PostType.Document)
        {
            return NotFound();
        }
    ...
    /// <param name="request">Request</param>
    /// <returns>Return the result</returns>
    [HttpPatch("v1/MyDocumentSearch"), Authorize]
    [ProducesResponseType(typeof(SingleResponse), (int)HttpStatusCode.OK)]
    public async Task<IActionResult> Search([FromBody] MyDocumentSearchR request)
    {
        request.Analyze(HttpContext);
        var response = await _mediator.Send(request);
        return Ok(response);
