...
public class SubPostSearchH : BaseSettingH, IRequestHandler<SubPostSearchR, SingleResponse>
{
    ...
    public async Task<SingleResponse> Handle(SubPostSearchR request, CancellationToken cancellationToken)
    {
        ...
        var q = _context.Available<DocumentSubPost>(false).Where(p => p.PostId == postId)
            .Select(subPost => new ChapterResponse
            {
                ...
                CommentCount = _context.Available<DocumentSubPostComment>(false).Count(p => p.PostId == subPost.Id)
            });
    ...
    /// <returns>Return the result</returns>
    private async Task<DocumentSubSortedSearchRsp> SortedSearch(Guid postId, bool descending, int pagNum, int pageSize, List<Guid> ids)
    {
        var res = new DocumentSubSortedSearchRsp();
        ...
        try
        {
            ...
            var req = new DocumentSubSortedSearchReq
            {
