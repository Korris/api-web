...
using Mcsg.Api.Areas.Document.Extensions;
...
public partial class PostLoadFeedH : BaseMinioH, IRequestHandler<PostLoadFeedR, SingleResponse>
{
    ...
    public async Task<SingleResponse> Handle(PostLoadFeedR request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var q = _context.Available<DocumentPost>(false);
            ...
            {
                q = q.Where(p => p.DocumentTagPosts.Any(tp => (tp.Tag.Name + "") == hashtag));
            }
            ...
            if (request.Hides != null && request.Hides.Count > 0)
            {
                ...
                q = q.Where(p => p.DocumentSubPosts.Any(sp => !sp.IsDelete));
            }
            ...
            q = q.Where(p => !p.DocumentPostHides.Any(h => h.UserId == request.UserId && !h.IsDelete));

            q = q.OrderByDescending(p => p.DocumentSubPosts.Max(sp => sp.CreatedOn) > p.CreatedOn
                ? p.DocumentSubPosts.Max(sp => sp.CreatedOn)
                : p.CreatedOn);

            // Favorite
            if (request.IsFavorite && request.UserId != null)
            {
                q = q.Where(p => p.DocumentPostFavorites.Any(f => f.UserId == request.UserId && !f.IsDelete));
            }
            #endregion
