...
using Mcsg.Api.Areas.Document.Filters;
...
/// Handler
/// </summary>
public class MyDocumentSearchH : BaseSettingH, IRequestHandler<MyDocumentSearchR, SingleResponse>
{
    #region -- Methods --
    ...
    public MyDocumentSearchH(IMcsgContext context, ISetting setting) : base(context, setting) { }
    ...
    public async Task<SingleResponse> Handle(MyDocumentSearchR request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var q = _context.Available<DocumentPost>().AsNoTracking().Where(p => p.UserId == userId);
            ...
            List<Guid> postIds = [];
            var viewResponse = new DocumentPostViewRsp();
                ...
                foreach (var i in request.Sort)
                {
                    ...
                    else if (i.Field.Equals("NumberOfViews", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (viewResponse != null && viewResponse.Items != null)
                        {
                            viewCounts = viewResponse.Items.ToDictionary(v => v.PostId + "", v => v.NumberOfViews);
                            listIds = viewResponse.Items.Select(v => v.PostId).ToList();
                            q = _context.Available<DocumentPost>().AsNoTracking().Where(p => p.UserId == userId);
    ...
    private async Task<DocumentPostViewRsp> GetPostViewFrAna(Guid userId, int pageNumber, int pageSize, bool isAscending, string? keyWord, List<Guid> postIds, List<PostPermission> permissions, DateTime? frDate, DateTime? toDate)
    {
        var res = new DocumentPostViewRsp();
        ...
        try
        {
            ...
            var request = new DocumentPostViewReq
            {
