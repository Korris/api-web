...
public partial class PostService : BaseMinioS, IPostService
{
    ...
    public async Task<PagedResponse<PostSeriesTopResponse>> GetTopSeriesAsync(PostType type, DocumentPostListSeriesR request)
    {
        ...
        {
            q = q.Where(p => p.DocumentTagPosts.Select(p => p.Tag.Name).Contains(request.HashTag));
        }

        var dbFeed = await q.Include(p => p.User)
            .Include(p => p.DocumentSubPosts)
            .Include(p => p.DocumentTagPosts).ThenInclude(p => p.Tag)
            .Select(p => new PostSeriesTopQueryDbResponse
            {
                ...
                LatestCreatedOn = p.DocumentSubPosts.Max(p => p.PublishDate),
                Tags = p.DocumentTagPosts.Select(p => p.Tag.Name).ToArray(),
                Status = p.Status,
                Hide = p.Hide
            })
