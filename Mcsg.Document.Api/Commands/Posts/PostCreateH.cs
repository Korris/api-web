...
public class PostCreateH : BaseMinioH, IRequestHandler<PostCreateR, SingleResponse>
{
    ...
    public async Task<SingleResponse> Handle(PostCreateR request, CancellationToken cancellationToken)
    {
        ...
        // Create
        var ett = DocumentPost.Create(request.Title, request.Content, request.ThumbnailUrl, profileName, request.CustomNote, userId);
        await _context.DocumentPosts.AddAsync(ett);
        ...
        if (request.MetaData != null)
        {
            ...
            result.MetaData = await _metaDataService.AddMetaDataToObject<DocumentPost>(request.MetaData, ett.Id);
        }
