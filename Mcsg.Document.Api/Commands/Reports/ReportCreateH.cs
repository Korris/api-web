...
namespace Mcsg.Document.Api.Commands;
    ...
    public async Task<SingleResponse> Handle(ReportCreateR request, CancellationToken cancellationToken)
    {
        ...
        switch (type)
        {
            case EntityType.Post:
                var post = await _context.Available<DocumentPost>().FirstOrDefaultAsync(p => p.Id == entityId, cancellationToken);
                if (post == null)
                {
            ...
            case EntityType.SubPost:
                var subPost = await _context.Available<DocumentSubPost>().FirstOrDefaultAsync(p => p.Id == entityId, cancellationToken);
                if (subPost == null)
                {
                    var t = new List<DicDto> { new() { Key = nameof(request.EntityId).ToCamelCase(), Value = entityId } };
                    return res.SetError(nameof(E002), E002, t);
            ...
            case EntityType.CommentPost:
                var commentPost = await _context.Available<DocumentPostComment>().FirstOrDefaultAsync(p => p.Id == entityId, cancellationToken);
                if (commentPost == null)
                {
                    var t = new List<DicDto> { new() { Key = nameof(request.EntityId).ToCamelCase(), Value = entityId } };
                    return res.SetError(nameof(E002), E002, t);
            ...
            case EntityType.CommentSubPost:
                var commentSubPost = await _context.Available<DocumentSubPostComment>().FirstOrDefaultAsync(p => p.Id == entityId, cancellationToken);
        ...
        var ett = await _context.Available<DocumentReport>().FirstOrDefaultAsync(p => p.EntityId == entityId, cancellationToken);
        if (ett != null)
        {
            var hasDetail = await _context.Available<DocumentReportDetail>().AnyAsync(p => p.ReportId == ett.Id && p.UserId == userId, cancellationToken);
        ...
        {
            // Create Report
            ett = DocumentReport.Create(entityId, type, userId);
            await _context.DocumentReports.AddAsync(ett, cancellationToken);
        }

        // Create ReportDetail
        var reasonType = request.ReasonType.ToEnum(ReasonType.Other);
        var ettDetail = DocumentReportDetail.Create(ett.Id, reasonType, request.ReasonText, userId);
        await _context.DocumentReportDetails.AddAsync(ettDetail, cancellationToken);
