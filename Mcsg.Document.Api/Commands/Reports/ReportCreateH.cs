    ...
    public async Task<SingleResponse> Handle(ReportCreateR request, CancellationToken cancellationToken)
    {
        ...
        switch (type)
        {
            case EntityType.Post:
                var post = await _context.DocumentPostAvailable.FirstOrDefaultAsync(p => p.Id == entityId, cancellationToken);
                if (post == null)
                {
            ...
            case EntityType.SubPost:
                var subPost = await _context.DocumentSubPostAvailable.FirstOrDefaultAsync(p => p.Id == entityId, cancellationToken);
                if (subPost == null)
                {
                    var t = new List<DicDto> { new() { Key = nameof(request.EntityId).ToCamelCase(), Value = entityId } };
                    return res.SetError(E002, M002, t);
            ...
            case EntityType.CommentPost:
                var commentPost = await _context.DocumentPostCommentAvailable.FirstOrDefaultAsync(p => p.Id == entityId, cancellationToken);
                if (commentPost == null)
                {
                    var t = new List<DicDto> { new() { Key = nameof(request.EntityId).ToCamelCase(), Value = entityId } };
                    return res.SetError(E002, M002, t);
            ...
            case EntityType.CommentSubPost:
                var commentSubPost = await _context.DocumentSubPostCommentAvailable.FirstOrDefaultAsync(p => p.Id == entityId, cancellationToken);
        ...
        var ett = await _context.DocumentReportAvailable.FirstOrDefaultAsync(p => p.EntityId == entityId, cancellationToken);
        if (ett != null)
        {
