...
public class PostReportCreateH : BaseH, IRequestHandler<PostReportCreateR, SingleResponse>
{
    ...
    public async Task<SingleResponse> Handle(PostReportCreateR request, CancellationToken cancellationToken)
    {
        ...
        var hasPostReport = await _context.DocumentPostReports.AnyAsync(p => p.PostId == request.PostId && p.UserId == userId, cancellationToken);
        if (hasPostReport)
        {
        ...
        var ett = DocumentPostReport.Create(request.PostId, userId, reasonType, request.ReasonText);
        await _context.DocumentPostReports.AddAsync(ett, cancellationToken);
